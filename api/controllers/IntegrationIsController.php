...
//            $request->taskUuid; // null
    ...
    private function createComment($data)
    {
        ...
        // Интерсвязь шлёт нам уведомления на любое событие, в том числе и на те которые мы поджигаем сами
        // проверяем наличие коментария в нашей системе, потому что алгоритм такой:
        // создаём коментарий в нашей системе(без extId), отправляем запрос на создание в Интерсвязь, получаем ответ,
        // из ответа получаем extId, сохраняем его в коментарий который создали. После этого(в это же время) Интерсвязь
        // шлёт нам уведомление о нашем же коментарии, и если он у нас есть мы не должны его создавать по новой.
        $comment = Comments::findOne([
            'extParentId' => $request->extId,
            'extId' => $data['id'],
            'integrationClass' => self::class,
            'extParentType' => 'null',
        ]);
        ...
        if ($comment != null) {
            // мы уже сами создали коментарий в нашей системе, уведомление интерсвязи игнорируем
            Yii::error('Получили уведомление о новом коментарии который уже есть в нашей системе. (extId='
                . $comment->extId . ')', self::LOG_TAG);
            return true;
        }
