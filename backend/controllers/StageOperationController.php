...
use backend\models\StageOperationSearch;
 ...
 * @link     http://www.toirus.ru
 */
class StageOperationController extends Controller
{
    ...
    {
        $searchModel = new StageOperationSearch();
    ...
    public function actionMove()
    {
        ...
        if ($action==9) {
            $template = StageOperation::find()->where(['_id' => $id])->one();
            if ($template) {
                if ($template->delete())
                    return Errors::OK;
                return Errors::ERROR_SAVE;
     ...
     * @param integer $id Id
     *
     * @return StageOperation the loaded model
     * @throws NotFoundHttpException if the model cannot be found
     */
    protected function findModel($id)
    {
        if (($model = StageOperation::findOne($id)) !== null) {
            return $model;
        } else {
